...
export function Navbar(props: NavbarProps) {
  ...
  if (isDarkMode) color = darkModeColor;
...
function DesktopMenu({position}: DesktopMenuProps) {
  ...
  return (
    <nav className="relative mx-14 text-sm max-md:hidden flex gap-8">
      {menu.items.map(item => (
...
function MenuItemWithDropdown({item}: {item: MenuItemConfig}) {
  const navigate = useNavigate();
  const handleClick = () => {
    if (isAbsoluteUrl(item.action)) {
      window.open(item.action, item.target ?? '_self')?.focus();
    } else if (item.action) {
  ...
  return (
    <div className="relative group">
      <button
        onClick={handleClick}
        className="opacity-90 hover:underline hover:opacity-100 flex items-center gap-1"
      >
      ...
      {hasChildren && (
        <div className="absolute left-0 top-full hidden group-hover:block bg-background shadow-lg rounded-2xl mt-2 min-w-[180px] z-50">
          {item.children.map(child => (
            <button
              key={child.id}
              onClick={() => {
                if (isAbsoluteUrl(child.action)) {
                  window.open(child.action, child.target ?? '_self')?.focus();
                } else if (child.action) {
                  navigate(child.action);
                }
              }}
              className="block w-full text-left px-4 py-2 hover:bg-accent hover:text-accent-foreground"
            >
              <Trans message={child.label} />
            </button>
          ))}
        </div>
      )}
...
function resolveLogoColor({
  ...
  if (logoColor != null) return logoColor;
  if (isDarkMode) return 'light';
  if (navbarColor === 'bg' || navbarColor === 'bg-alt') return 'dark';
  return 'light';
}
