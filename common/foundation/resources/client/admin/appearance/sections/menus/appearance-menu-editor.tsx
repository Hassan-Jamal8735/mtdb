...
function MenuEditorSection({menuFormPath}: MenuEditorFormProps) {
  ...
  const menuPositions = useMemo(() => {
    ...
    if (has_mobile_app) positions.push('mobile-app-about');
    ...
    <Fragment>
      <div className="flex flex-shrink-0 items-center justify-between gap-16">
        ...
        <DialogTrigger
          ...
          placement="right"
          offset={20}
          onClose={(updatedItems?: MenuItemConfig[]) => {
            if (updatedItems) {
              form.setValue(formPath, updatedItems, {shouldDirty: true});
              navigate(`${updatedItems.length - 1}`);
            }
          }}
        >
          <Button
          ...
          {/* Pass existing items into the dialog for parent selector */}
          <AddMenuItemDialog menu={{items}} />
        </DialogTrigger>
      </div>
        ...
        {items.map((item, index) => (
          <div key={item.id}>
            ...
            {item.children?.map((child, cIndex) => (
              <div key={child.id} className="pl-12 mt-4">
                <MenuListItem
                  item={child}
                  items={item.children}
                  index={cIndex}
...
function DeleteMenuTrigger() {
  ...
  const menus = useWatch({control: form.control, name});
