...
class TmdbApi
{
    ...
    public function __construct()
    {
        ...
        $this->includeAdult = settings('tmdb.includeAdult', false);
    }
    ...
    public function getPerson(Person $person): array|null
    {
        ...
        if (settings('content.automate_filmography')) {
    ...
    public function browse($page = 1, $type = 'movie', $queryParams = []): array
    {
        ...
        // Expand "Europe" filter into multiple TMDb production countries
        if (isset($queryParams['with_production_countries']) && $queryParams['with_production_countries'] === 'EU') {
            $queryParams['with_production_countries'] = implode(',', [
                'FR','DE','IT','ES','PL','SE','NO','FI','DK','NL','BE','PT','AT','CH',
                'CZ','SK','HU','IE','GR','IS','LU','SI','HR','BG','EE','LV','LT','RO',
                'UA','RS','BA','MD','AL','MT'
            ]);
        }


        // Inject global animation genre filter (ID: 16)
    if (!isset($queryParams['with_genres'])) {
